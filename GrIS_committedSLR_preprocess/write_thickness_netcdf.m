function write_thickness_netcdf(md_mesh_elements, md_mesh_x, md_mesh_y, ...
      time, thickness, output_netcdf_filename, variable_name)
   ...
   % interpolate dh_total onto grid
   thickness_grid = zeros(length(ygrid), length(xgrid), length(time));
   for j = 1:length(time)
       [~, thickness_grid(:,:,j)] = evalc('InterpFromMeshToGrid(md_mesh_elements, md_mesh_x, md_mesh_y, thickness(:,j), xgrid, ygrid, NaN);');
   end
   ...
   ncwrite(output_netcdf_filename,variable_name, thickness_grid)
   ncwriteatt(output_netcdf_filename,variable_name,'grid_mapping','Polar_Stereographic')
   ncwriteatt(output_netcdf_filename,variable_name,'standard_name','land_ice_thickness')
   ncwriteatt(output_netcdf_filename,variable_name,'units','m')
